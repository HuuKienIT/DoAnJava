...
//		navMenu.setBackground(new Color(0, 184, 148));
		...
		dangxuat.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int output = JOptionPane.showConfirmDialog(contentPane, "Bạn có muốn đăng xuất không", "",
	...
	public void HienSuDung() {
		if (!BUS.PhanQuyenBUS.KTQuyen(nv.getId_nv(), 6)) {
			panel_SP.setVisible(false);
		}
		if (!BUS.PhanQuyenBUS.KTQuyen(nv.getId_nv(), 1)) {
			panel_BH.setVisible(false);
		}
		if(!BUS.PhanQuyenBUS.KTQuyen(nv.getId_nv(), 9)) {
			panel_KH.setVisible(false);
		}
		if (!BUS.PhanQuyenBUS.KTQuyen(nv.getId_nv(), 3)) {
			panel_NV.setVisible(false);
		}
		if(!BUS.PhanQuyenBUS.KTQuyen(nv.getId_nv(), 8)) {
			panel_TK.setVisible(false);
		}
		if (!BUS.PhanQuyenBUS.KTQuyen(nv.getId_nv(), 2)) {
			panel_NH.setVisible(false);
		}
		if (!BUS.PhanQuyenBUS.KTQuyen(nv.getId_nv(), 5)) {
