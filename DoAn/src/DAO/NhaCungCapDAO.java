...
public class NhaCungCapDAO {
	...
	public static int themNCC(NhaCungCapModel ncc) {
		int n = -1;
		try {
			String sql = "insert into nhacungcap values('"+ncc.id_ncc+"','"+ncc.ten_ncc+"','"+ncc.sodth+"','"+ncc.email+"')";
			mySQLHelper helper = new mySQLHelper();
			helper.open();
	...
	public static int suaNCC(NhaCungCapModel ncc) {
		int n = -1;
		try {
			String sql="update nhancungcap set ten_nh='"+ncc.ten_ncc+"' sodth='"+ncc.sodth+"' email='"+ncc.email+"' where id_nh='"+ncc.id_ncc+"'";
			mySQLHelper helper=new mySQLHelper();
			helper.open();
			n=helper.executeUpdate(sql);
		} catch (Exception e) {
